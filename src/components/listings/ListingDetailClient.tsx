        ...
        <div>
          ...
          <div className="mt-6 rounded-xl p-4" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border)' }}>
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                { label: 'No Scratches', value: !listing.hasScratch, optional: false },
                { label: 'Functional', value: listing.isFunctional, optional: false },
                { label: 'Complete', value: listing.hasCompleteParts, optional: false },
                { label: 'Original Box', value: listing.hasOriginalBox, optional: true },
                { label: 'Under Warranty', value: listing.hasWarranty, optional: true },
              ].map(item => (
                <div key={item.label} className="flex items-center gap-2 text-xs">
                  <div className={`w-4 h-4 rounded-full flex items-center justify-center flex-shrink-0 ${item.value ? 'bg-green-500/20' : item.optional ? 'bg-gray-500/10' : 'bg-red-500/20'}`}>
                    <span style={{ color: item.value ? 'var(--green)' : item.optional ? 'var(--text-muted)' : 'var(--red)', fontSize: '10px' }}>
