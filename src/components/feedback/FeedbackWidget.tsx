...
export function FeedbackWidget() {
  ...
  const [type, setType] = useState<'bug' | 'cadangan' | 'lain-lain'>('bug')
  ...
  return (
    <>
      ...
      <button
        ...
        aria-label="Hantar maklum balas"
      >
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4" style={{ backgroundColor: 'rgba(0,0,0,0.6)' }}>
          <div className="w-full max-w-md rounded-2xl p-6" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border)' }}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold text-lg">Maklum Balas Beta</h3>
            ...
            {sent ? (
              <div className="text-center py-8">
                ...
                <p className="font-semibold" style={{ color: 'var(--green)' }}>Terima kasih! Maklum balas diterima.</p>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <label className="block text-xs font-medium mb-2" style={{ color: 'var(--text-secondary)' }}>Jenis</label>
                  <div className="flex gap-2">
                    {(['bug', 'cadangan', 'lain-lain'] as const).map(t => (
                      <button
                        key={t} type="button"
                        onClick={() => setType(t)}
                      ...
                      >
                        {t === 'bug' ? '🐛 Bug' : t === 'cadangan' ? '💡 Cadangan' : '💬 Lain-lain'}
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  <label className="block text-xs font-medium mb-2" style={{ color: 'var(--text-secondary)' }}>Mesej</label>
                  <textarea
                    required
                    value={message}
